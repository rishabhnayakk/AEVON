...
function setupMobileNavigation() {
    ...
    document.addEventListener('click', (e) => {
        if (sidebar && sidebar.classList.contains('open') && e.target.closest('.nav-item')) {
            sidebar.classList.remove('open');
            overlay.classList.remove('show');
        }
    });
...
function renderSidebar(activePage = '') {
    ...
    const roleLabel = document.getElementById('role-label') || document.getElementById('user-role-label');
    ...
    if (sidebarHeader) {
        ...
        roleLabel.textContent = user.role === 'admin' ? 'Administrator' :
                               user.role === 'teacher' ? 'Teacher Panel' : 'Student Panel';
    }
