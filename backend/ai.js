...
function generateFallbackAnalysis(data) {
    const allSubjectsZero = data.subjectAverages.length > 0 && data.subjectAverages.every(s => s.avg === 0);
    ...
    const predictions = data.subjectAverages.map(s => ({
        subject: s.name,
        currentAvg: s.avg,
        predictedNext: (isNewStudent && !isCriticalZero) ? 70 : Math.min(100, Math.round(s.avg * 1.05)),
        trend: isCriticalZero ? 'critical' : isNewStudent ? 'new_student' : (s.avg > 60 ? 'stable' : 'needs_improvement'),
    }));

    const minMark = data.subjectAverages.length ? Math.min(...data.subjectAverages.map(s => s.avg)) : 100;
    ...
    const weakSubjects = (!isNewStudent && !isCriticalZero)
        ? data.subjectAverages.filter(s => s.avg < 50).map(s => s.name)
        : [];
    const strongSubjects = (!isNewStudent && !isCriticalZero)
    ...
    if (isCriticalZero) {
        recommendations = data.subjectAverages.map(s => ({
            ...
            tips: [
                ...
                '🗓️ Create a daily study schedule for this subject',
    ...
    if (isCriticalZero) {
        ...
        if (data.overallAverage >= 75) generalAdvice.push('🌟 Excellent performance! Keep it up.');
        else if (data.overallAverage >= 50) generalAdvice.push('📈 Good progress. Focus on weak areas.');
        else generalAdvice.push('‼️ Needs immediate attention. Create a study plan.');
    }

    return {
        predictions, riskLevel, riskReasons: reasons,
        recommendations, generalAdvice, strengths: strongSubjects, weaknesses: weakSubjects,
    };
}
        ...
        .map(s => {
            ...
            if (criticalSubjects.length > 0 || s.subjectMarks.length === 0) {
                ...
                if (s.subjectMarks.length === 0) {
                    ...
                    criticalSubjects.forEach(m => reasons.push(`Critical in ${m.subjectName} (${m.percentage}%)`));
                }
            } else if (lowSubjects.length > 0) {
                riskLevel = 'medium';
                lowSubjects.forEach(m => reasons.push(`Low marks in ${m.subjectName} (${m.percentage}%)`));
            }

            const subjectsAtRisk = s.subjectMarks.length === 0 
                ? ['All Subjects (No Data)']
                : (riskLevel === 'high' ? criticalSubjects : lowSubjects).map(m => `${m.subjectName} (${m.percentage}%)`);
