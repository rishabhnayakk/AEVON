...
const { MONGO_URI, SESSION_SECRET, PORT } = require('./config');
const { User, Class, Student, Subject, Mark, Exam, ExamConfig, Question, ExamAttempt, Notification, StudyMaterial } = require('./models');
const { analyzeStudent, analyzeAtRisk, chatAssistant } = require('./ai');
...
async function filterRealMarks(marks, onlyPublished = false) {
    ...
    if (onlyPublished) {
        ...
        const publishedConfigs = await mongoose.model('ExamConfig').find({ resultsVisible: true }, 'examId');
    ...
    Object.values(groups).forEach(group => {
        ...
        const examMarks = group.filter(m => {
            ...
            if (onlyPublished) {
                return publishedIds.has(m.examId.toString()) || publishedIds.has(m.examId?._id?.toString());
            }
            return true;
        });
...
async function checkAndAwardBadges(studentId) {
    try {
        ...
        if (!hasBadge('Math Wizard')) {
            const mathMarks = filteredMarks.filter(m => m.subjectId && m.subjectId.name && m.subjectId.name.toLowerCase().includes('math'));
            if (mathMarks.length > 0) {
                const totalObtained = mathMarks.reduce((sum, m) => sum + m.marksObtained, 0);
                const totalMax = mathMarks.reduce((sum, m) => sum + m.maxMarks, 0);
                const mathAvg = totalMax > 0 ? (totalObtained / totalMax) * 100 : 0;
                if (mathAvg >= 85) {
                    newBadges.push({
                        name: 'Math Wizard',
                        description: 'Achieve an average of 85% or higher in Math subjects.',
                        icon: '🧙‍♂️'
                    });
                }
            }
        ...
        if (!hasBadge('Perfect Score')) {
            const perfectScoreExists = filteredMarks.some(m => Math.round((m.marksObtained / m.maxMarks) * 100) >= 100);
        ...
        const examList = Object.values(examsGrouped)
            .map(g => {
                const totalObtained = g.marks.reduce((sum, m) => sum + m.marksObtained, 0);
        ...
        if (!hasBadge('Most Improved') && examList.length >= 2) {
            ...
            if (has15PercentJump) {
                newBadges.push({
                    name: 'Most Improved',
                    description: 'Improve your overall exam score by 15% or more between consecutive exams.',
                    icon: '📈'
                });
            }
        }
...
app.get('/api/students', requireLogin, async (req, res) => {
    let filter = (req.query.class_id && req.query.class_id !== 'all') ? { classId: req.query.class_id } : {};
    
    if (req.session.role === 'teacher') {
        const teacherClassIds = await getTeacherClasses(req.session.userId);
        if (req.query.class_id && req.query.class_id !== 'all') {
            if (!teacherClassIds.map(id => id.toString()).includes(req.query.class_id)) {
    ...
    const users = await User.find({ studentId: { $in: students.map(s => s._id) } });
    const userMap = {};
    users.forEach(u => userMap[u.studentId.toString()] = { id: u._id.toString(), isBlocked: u.isBlocked });
...
app.get('/api/students/:id', requireLogin, async (req, res) => {
    ...
    res.json({
        ...
        enrollment_no: s.enrollmentNo,
        class_id: s.classId?._id?.toString(), class_name: s.classId?.name || '',
    });
});
...
app.post('/api/students', requireAdmin, async (req, res) => {
    try {
        const s = await Student.create({
            ...
            enrollmentNo: req.body.enrollment_no, classId: req.body.class_id,
        });
        ...
        if (req.body.email) {
            ...
            if (!existing) {
                ...
                await User.create({ username: req.body.email, password: pwd, role: 'student', studentId: s._id });
            }
        }
        ...
        res.json({ id: s._id.toString(), message: 'Created', marks_seeded: markInserts.length });
...
app.get('/api/marks', requireLogin, async (req, res) => {
    try {
        ...
        if (req.query.class_id && req.query.class_id !== 'all') {
            const studentsInClass = await Student.find({ classId: req.query.class_id });
            const studentIds = studentsInClass.map(s => s._id);
            filter.studentId = { $in: studentIds };
        }

        const marks = await Mark.find(filter).populate('subjectId examId');
        res.json({ marks: marks.map(m => ({
            id: m._id.toString(),
            ...
            subject_id: m.subjectId ? (m.subjectId._id || m.subjectId).toString() : null,
            subject_name: m.subjectId?.name || '', 
            percentage: Math.round((m.marksObtained / (m.maxMarks || m.examId?.totalMarks || 100)) * 100)
        }))});
    } catch (e) {
...
app.get('/api/marks/student/:id', requireLogin, async (req, res) => {
    try {
        ...
        if (req.session.role === 'teacher') {
            if (!student.classId) {
                return res.status(403).json({ error: 'Access denied: This student is not assigned to any class' });
            }
            const teacherClassIds = await getTeacherClasses(req.session.userId);
            if (!teacherClassIds.map(id => id.toString()).includes(student.classId._id.toString())) {
                return res.status(403).json({ error: 'Access denied: This student is not in your assigned class' });
            }
        }
    ...
    try {
        ...
        if (exam_id) {
            ...
            if (exam.status !== 'completed') {
                return res.status(400).json({ error: 'Marks can only be uploaded after exam completion.' });
            }
            if (marksValue > exam.totalMarks) {
                return res.status(400).json({ error: `Marks (${marksValue}) cannot exceed total exam marks (${exam.totalMarks})` });
            }
        }

        const maxMarks = req.body.max_marks || (exam_id ? (await Exam.findById(exam_id)).totalMarks : 100);
...
app.post('/api/marks/bulk', requireLogin, async (req, res) => {
    try {
        ...
        if (operations.length > 0) {
            ...
            const studentIds = [...new Set((req.body.marks || []).map(m => m.student_id))];
...
app.put('/api/marks/:id', requireLogin, async (req, res) => {
    try {
        ...
        if (mark.examId && mark.examId.status !== 'completed') {
            return res.status(400).json({ error: 'Marks can only be updated after exam completion.' });
        }

        const marksObtained = Number(req.body.marks_obtained);
        const maxMarks = mark.examId ? mark.examId.totalMarks : (req.body.max_marks || mark.maxMarks);

        if (marksObtained < 0) return res.status(400).json({ error: 'Marks cannot be negative' });
        if (mark.examId && marksObtained > mark.examId.totalMarks) {
            return res.status(400).json({ error: `Marks cannot exceed exam total marks (${mark.examId.totalMarks})` });
        }
...
app.get('/api/analytics/classes', requireLogin, async (req, res) => {
    ...
    for (const c of classes) {
        ...
        result.push({ id: c._id.toString(), name: c.name, student_count: count, teacher_ids: c.teacherIds.map(t => t.toString()) });
    }
...
app.get('/api/analytics/subjects', requireLogin, async (req, res) => {
    ...
    res.json({ subjects: subjects.map(s => ({ id: s._id.toString(), name: s.name, code: s.code })) });
});
...
app.get('/api/analytics/overview', requireLogin, async (req, res) => {
    try {
        ...
        const allStudentsWithClass = await Student.find(studentFilter).populate('classId');
        ...
        if (filteredMarks.length) {
            avg = Math.round(filteredMarks.reduce((s, m) => s + (m.marksObtained / m.maxMarks) * 100, 0) / filteredMarks.length);
        }
        ...
        const subject_performance = Object.entries(subjectMap).map(([name, vals]) => ({
            subject_name: name, avg_percentage: Math.round(vals.reduce((a, b) => a + b, 0) / vals.length)
        ...
        for (const s of allStudentsWithClass) {
            ...
            const sMarks = filteredMarks.filter(m => m.studentId.toString() === s._id.toString());
            if (sMarks.length) {
                if (!classMap[cn]) classMap[cn] = [];
                const a = sMarks.reduce((sum, m) => sum + (m.marksObtained / m.maxMarks) * 100, 0) / sMarks.length;
                classMap[cn].push(a);
            }
        }
        const class_performance = Object.entries(classMap).map(([name, avgs]) => ({
            class_name: name, avg_percentage: Math.round(avgs.reduce((s, a) => s + a, 0) / avgs.length)
        })).sort((a, b) => b.avg_percentage - a.avg_percentage);
        ...
        for (const s of students) {
            const sMarks = filteredMarks.filter(m => m.studentId.toString() === s._id.toString());
            if (sMarks.length) {
                const a = Math.round(sMarks.reduce((sum, m) => sum + (m.marksObtained / m.maxMarks) * 100, 0) / sMarks.length);
                studentAvgs.push({ id: s._id.toString(), name: s.name, avg_percentage: a });
            }
        }
        const top_students = [...studentAvgs].sort((a, b) => b.avg_percentage - a.avg_percentage).slice(0, 5);
        ...
        res.json({
            ...
            class_performance,
            top_students,
            student_performance: studentAvgs.sort((a, b) => b.avg_percentage - a.avg_percentage)
        });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/analytics/class/:id', requireLogin, async (req, res) => {
    ...
    for (const s of students) {
        const sMarks = filteredMarks.filter(m => m.studentId.toString() === s._id.toString());
        const avg = sMarks.length ? Math.round(sMarks.reduce((sum, m) => sum + (m.marksObtained / m.maxMarks) * 100, 0) / sMarks.length) : 0;
        studentData.push({ id: s._id.toString(), name: s.name, enrollment_no: s.enrollmentNo, avg_percentage: avg });
    }
    studentData.sort((a, b) => b.avg_percentage - a.avg_percentage);

    // Subject avgs
    const subjectData = subjects.map(sub => {
        const sMarks = filteredMarks.filter(m => m.subjectId?._id.toString() === sub._id.toString());
        const avg = sMarks.length ? Math.round(sMarks.reduce((s, m) => s + (m.marksObtained / m.maxMarks) * 100, 0) / sMarks.length) : 0;
        return { name: sub.name, avg_percentage: avg };
    }).sort((a, b) => b.avg_percentage - a.avg_percentage);

    // Grade distribution
    const allPcts = filteredMarks.map(m => (m.marksObtained / m.maxMarks) * 100);
    ...
    res.json({ students: studentData, subjects: subjectData, grade_distribution });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/analytics/student/:id', requireLogin, async (req, res) => {
    ...
    const allClassStudents = await Student.find({ classId: student.classId._id });

    // Overall avg for context
    const avgMarks = filteredMarks.length ? Math.round(filteredMarks.reduce((s, m) => s + (m.marksObtained / m.maxMarks) * 100, 0) / filteredMarks.length) : 0;
    ...
    for (const cs of allClassStudents) {
        ...
        const a = filteredCSMarks.length ? filteredCSMarks.reduce((s, m) => s + (m.marksObtained / m.maxMarks) * 100, 0) / filteredCSMarks.length : 0;
    ...
    filteredMarks.forEach(m => {
        ...
        if (name) {
            if (!subjMap[name]) subjMap[name] = { totalObtained: 0, totalMax: 0, count: 0 };
    ...
    res.json({
        student: { 
            ...
            user_id: req.session.role === 'admin' ? (await User.findOne({ studentId: student._id }))?._id : null,
...
app.get('/api/ai/predict/:id', requireLogin, async (req, res) => {
    try {
        ...
        if (!effectiveSubjectAverages.length) {
            ...
            effectiveSubjectAverages = classSubjects.map(s => ({ name: s.name, avg: 0 }));
        }

        const data = {
            name: student.name, className: student.classId?.name || '',
            enrollmentNo: student.enrollmentNo,
            marks: marks.map(m => ({
                subjectName: m.subjectId?.name || '',
                marksObtained: m.marksObtained, maxMarks: m.maxMarks
            })),
            subjectAverages: effectiveSubjectAverages,
            overallAverage: effectiveSubjectAverages.length ? Math.round(effectiveSubjectAverages.reduce((s, a) => s + a.avg, 0) / effectiveSubjectAverages.length) : 0,
        };
        ...
        res.json({
            predictions: (result.predictions || []).map(p => ({
                subject_name: p.subject, current_average: p.currentAvg,
                predicted_percentage: p.predictedNext, trend: p.trend
            })),
            risk_level: result.riskLevel, risk_reasons: result.riskReasons || []
        });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/ai/recommendations/:id', requireLogin, async (req, res) => {
    try {
        ...
        if (!effectiveSubjectAverages2.length) {
            const classSubjects2 = await Subject.find({ classId: student.classId._id });
            effectiveSubjectAverages2 = classSubjects2.map(s => ({ name: s.name, avg: 0 }));
        }

        const data = {
            name: student.name, className: student.classId?.name || '',
            enrollmentNo: student.enrollmentNo,
            marks: marks.map(m => ({ subjectName: m.subjectId?.name || '', marksObtained: m.marksObtained, maxMarks: m.maxMarks })),
            subjectAverages: effectiveSubjectAverages2,
            overallAverage: effectiveSubjectAverages2.length ? Math.round(effectiveSubjectAverages2.reduce((s, a) => s + a.avg, 0) / effectiveSubjectAverages2.length) : 0,
        };
        const result = await analyzeStudent(data);
        res.json({
...
app.get('/api/ai/at-risk', requireLogin, async (req, res) => {
    try {
        ...
        const studentIds = students.map(s => s._id);
        const allMarks = await Mark.find({ studentId: { $in: studentIds } }).populate('subjectId');
        ...
        const studentsData = students.map(s => {
            ...
            const overallAverage = subjectMarks.length ? Math.round(subjectMarks.reduce((acc, m) => acc + m.percentage, 0) / subjectMarks.length) : 0;

            return {
                name: s.name, className: s.classId?.name || '',
                overallAverage, 
                subjectMarks
            };
        });
...
app.post('/api/ai/chat', requireLogin, async (req, res) => {
    try {
        ...
        if (role === 'student') {
            const user = await User.findById(req.session.userId);
            if (!user.studentId) return res.status(400).json({ error: 'No student profile linked.' });
            const student = await Student.findById(user.studentId);
            const marks = await Mark.find({ studentId: student._id }).populate('subjectId');
            dataStr = `Student Name: ${student.name}. Marks: ` + marks.map(m => `${m.subjectId?.name}:${m.marksObtained}/${m.maxMarks}`).join(', ');
        } else if (role === 'admin') {
            dataStr = "You are the Admin. You have access to all system data.";
        } else {
            const students = await Student.find();
            const marks = await Mark.find().populate('subjectId');
            dataStr = "System Students Data:\n" + students.map(s => {
                const sMarks = marks.filter(m => m.studentId.toString() === s._id.toString());
...
app.post('/api/admin/toggle-block', requireAdmin, async (req, res) => {
    try {
        ...
        if (user.role === 'admin') return res.status(403).json({ error: 'Cannot block administrator' });

        user.isBlocked = !user.isBlocked;
        await user.save();
        res.json({ message: user.isBlocked ? 'User blocked' : 'User unblocked', is_blocked: user.isBlocked });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/teachers', requireAdmin, async (req, res) => {
    ...
    res.json({ teachers: teachers.map(t => ({ id: t._id.toString(), username: t.username })) });
});
...
app.put('/api/classes/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const c = await Class.findByIdAndUpdate(req.params.id, { teacherIds }, { new: true });
        res.json(c);
    } catch (e) { res.status(400).json({ error: e.message }); }
});
    ...
    try {
        ...
        res.json({
            subjects: subjects.map(s => ({
                ...
                class_id: s.classId?._id?.toString(), class_name: s.classId?.name || 'Unassigned'
            }))
        });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/subjects', requireAdmin, async (req, res) => {
    try {
        ...
        if (!name || !code || !class_id) return res.status(400).json({ error: 'Name, code, and class_id are required' });
...
app.get('/api/exams', requireLogin, async (req, res) => {
    try {
        let filter = {};
        if (req.query.class_id && req.query.class_id !== 'all') filter.classId = req.query.class_id;
        
        const exams = await Exam.find(filter).populate('classId');
        res.json({
            exams: exams.map(e => ({
                id: e._id.toString(), name: e.name, date: e.date,
...
app.patch('/api/exams/:id/status', requireLogin, async (req, res) => {
    try {
        ...
        const e = await Exam.findByIdAndUpdate(req.params.id, { status }, { new: true });
...
app.get('/api/exam-portal/config/:examId', requireLogin, async (req, res) => {
    try {
        const config = await ExamConfig.findOne({ examId: req.params.examId }).populate('subjects.subjectId');
...
app.post('/api/exam-portal/config', requireAdmin, async (req, res) => {
    try {
        const { exam_id, subjects, duration_minutes, starts_at, login_window_minutes, strict_forward_only, time_per_question_seconds, negative_marks } = req.body;
        if (!exam_id || !subjects || !Array.isArray(subjects) || subjects.length === 0) {
            return res.status(400).json({ error: 'exam_id and subjects array required' });
        }
        
        const formattedSubjects = subjects.map(s => {
            const tm = Number(s.total_marks);
            const tq = Number(s.total_questions);
            if (!tm || !tq) throw new Error('total_marks and total_questions required for each subject');
            return {
        ...
        res.json({ id: config._id.toString(), message: 'Exam configured successfully' });
    } catch (e) { res.status(400).json({ error: e.message }); }
});

// Toggle exam active/inactive
app.post('/api/exam-portal/config/:examId/toggle', requireAdmin, async (req, res) => {
    try {
        const config = await ExamConfig.findOne({ examId: req.params.examId }).populate('subjects.subjectId');
        if (!config) return res.status(404).json({ error: 'Exam not configured yet' });
        
        if (!config.isActive) {
            // Before activating, check if all questions are uploaded
            const questions = await Question.find({ examId: req.params.examId });
            for (const sub of config.subjects) {
                const subQuestions = questions.filter(q => q.subjectId.toString() === sub.subjectId._id.toString());
                if (subQuestions.length !== sub.totalQuestions) {
                    return res.status(400).json({ error: `Cannot activate: Subject ${sub.subjectId.name} requires ${sub.totalQuestions} questions, but has ${subQuestions.length}.` });
                }
            }
        }
        ...
        await config.save();
        res.json({ is_active: config.isActive, message: config.isActive ? 'Exam is now OPEN' : 'Exam is now CLOSED' });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// Toggle result visibility (admin publishes/hides results for students)
app.post('/api/exam-portal/config/:examId/toggle-results', requireAdmin, async (req, res) => {
    try {
        const config = await ExamConfig.findOne({ examId: req.params.examId });
        if (!config) return res.status(404).json({ error: 'Exam not configured yet' });
        config.resultsVisible = !config.resultsVisible;
        await config.save();
        res.json({ results_visible: config.resultsVisible, message: config.resultsVisible ? 'Results are now VISIBLE to students' : 'Results are now HIDDEN from students' });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// ── Questions (Admin) ────────────────────────────────────────
// Get all questions for an exam
app.get('/api/exam-portal/questions/:examId', requireLogin, async (req, res) => {
    try {
        const questions = await Question.find({ examId: req.params.examId }).populate('subjectId').sort('order');
        const isStudent = req.session.role === 'student';
        res.json({
            questions: questions.map(q => ({
                ...
                ...(isStudent ? {} : { 
                    correct_option: q.correctOptions && q.correctOptions.length === 1 ? q.correctOptions[0] : (q.correctOption !== undefined ? q.correctOption : -1),
                    correct_options: q.correctOptions && q.correctOptions.length > 0 ? q.correctOptions : (q.correctOption !== undefined && q.correctOption !== -1 ? [q.correctOption] : [])
                }),
            }))
        });
    } catch (e) { res.status(500).json({ error: e.message }); }
...
app.post('/api/exam-portal/questions', requireAdmin, async (req, res) => {
    try {
        const { exam_id, subject_id, question_text, options, correct_option, correct_options, image_data } = req.body;
        
        let correctOptionsArr = [];
        if (Array.isArray(correct_options)) {
            correctOptionsArr = correct_options.map(Number);
        } else if (correct_option !== undefined && correct_option !== null && correct_option !== '') {
            correctOptionsArr = [Number(correct_option)];
        }

        if (!exam_id || !subject_id || (!question_text && !image_data) || !options || options.length !== 4 || correctOptionsArr.length === 0) {
            return res.status(400).json({ error: 'All fields required: exam_id, subject_id, question_text (or image), 4 options, and at least one correct option' });
        }
...
app.put('/api/exam-portal/questions/:id', requireAdmin, async (req, res) => {
    try {
        const { question_text, options, correct_option, correct_options, image_data } = req.body;
        ...
        if ((!question_text && !hasImage) || !options || options.length !== 4 || correctOptionsArr.length === 0) {
            return res.status(400).json({ error: 'Question text or image is required, along with 4 options and at least one correct option.' });
        }
        ...
        if (image_data && typeof image_data === 'string' && image_data.startsWith('data:')) {
            ...
            } catch (err) { console.error('Failed to save question image:', err.message); }
        }
...
app.get('/api/exam-portal/available', requireLogin, async (req, res) => {
    try {
        if (req.session.role === 'student') {
            ...
            const exams = await Exam.find({ classId: student.classId }).populate('classId');
            const configs = await ExamConfig.find({ examId: { $in: exams.map(e => e._id) } }).populate('subjects.subjectId');
            const attempts = await ExamAttempt.find({ studentId: student._id });
            const configMap = Object.fromEntries(configs.map(c => [c.examId.toString(), c]));
            const attemptMap = Object.fromEntries(attempts.map(a => [a.examId.toString(), a]));
            ...
            res.json({ exams: exams.map(e => {
                ...
                return {
                    id: e._id.toString(), name: e.name, date: e.date, class_name: e.classId?.name,
                    is_configured: !!cfg,
                    is_active: cfg?.isActive || false,
                    results_visible: resultsVisible,
                    duration_minutes: cfg?.durationMinutes,
                    subjects: cfg?.subjects ? cfg.subjects.map(s => ({
                        subject_id: s.subjectId?._id?.toString(),
                        subject_name: s.subjectId?.name,
                        total_marks: s.totalMarks,
                        total_questions: s.totalQuestions,
            ...
            res.json({ exams: exams.map(e => {
                const cfg = configMap[e._id.toString()];
                return {
                    id: e._id.toString(), name: e.name, date: e.date, status: e.status,
                    class_id: e.classId?._id?.toString(), class_name: e.classId?.name,
                    is_configured: !!cfg, is_active: cfg?.isActive || false,
...
app.get('/api/exam-portal/start/:examId', requireLogin, async (req, res) => {
    try {
        ...
        const config = await ExamConfig.findOne({ examId: req.params.examId });
        if (!config) return res.status(404).json({ error: 'Exam not configured yet' });
        if (!config.isActive) return res.status(403).json({ error: 'This exam is not open yet' });

        // Enforce time window if set
        const now = new Date();
        if (config.startsAt && now < config.startsAt) {
            const opensIn = config.startsAt.toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' });
            return res.status(403).json({ error: `This exam has not started yet. It opens at ${opensIn}.` });
        }
        
        if (config.startsAt) {
            const deadline = new Date(config.startsAt.getTime() + (config.loginWindowMinutes || 15) * 60000);
            if (now > deadline) {
                return res.status(403).json({ error: 'The deadline to log in to this exam has passed.' });
            }
        }

        // Check for existing attempt
        const existing = await ExamAttempt.findOne({ examId: req.params.examId, studentId: student._id });
        if (existing) return res.status(409).json({ error: 'You have already submitted this exam', attempt: { score: existing.score, max_score: existing.maxScore } });

        const questions = await Question.find({ examId: req.params.examId }).sort('order');
...
app.post('/api/exam-portal/submit/:examId', requireLogin, async (req, res) => {
    try {
        ...
        const config = await ExamConfig.findOne({ examId: req.params.examId });
        if (!config || !config.isActive) return res.status(403).json({ error: 'Exam is not active' });
        ...
        (answers || []).forEach(a => {
            ...
            } else if (a.selectedOption !== undefined && a.selectedOption !== null && a.selectedOption !== -1 && a.selectedOption !== "-1") {
        ...
        config.subjects.forEach(s => {
            subjectScores[s.subjectId.toString()] = { score: 0, maxScore: s.totalMarks };
        ...
        const gradedAnswers = questions.map(q => {
            ...
            const correct = q.correctOptions && q.correctOptions.length > 0 
                ? q.correctOptions 
                : (q.correctOption !== undefined && q.correctOption !== -1 ? [q.correctOption] : []);
                
            const subCfg = config.subjects.find(s => s.subjectId.toString() === q.subjectId.toString());
        ...
        await ExamAttempt.create({
            examId: req.params.examId, studentId: student._id,
        ...
        for (const [subId, data] of Object.entries(subjectScores)) {
            try {
                await Mark.findOneAndUpdate(
                    { studentId: student._id, subjectId: subId, examId: req.params.examId },
        ...
        res.json({ message: 'Exam submitted successfully! Results will be available once published by the administrator.' });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/exam-portal/results/:examId', requireLogin, async (req, res) => {
    try {
        if (req.session.role === 'student') return res.status(403).json({ error: 'Access denied' });
        const attempts = await ExamAttempt.find({ examId: req.params.examId }).populate('studentId');
        res.json({ results: attempts.map(a => ({
            id: a._id,
            student_name: a.studentId?.name,
            student_id: a.studentId?._id,
            score: a.score, max_score: a.maxScore,
...
app.get('/api/exam-portal/attempt-details/:attemptId', requireLogin, async (req, res) => {
    try {
        const attempt = await ExamAttempt.findById(req.params.attemptId).populate('studentId');
        ...
        // Security check
        if (isStudent) {
            if (attempt.studentId._id.toString() !== req.session.userId && attempt.studentId.userId?.toString() !== req.session.userId) {
                // Check if user is the student
                const studentUser = await User.findById(req.session.userId);
                if (studentUser.studentId?.toString() !== attempt.studentId._id.toString()) {
                    return res.status(403).json({ error: 'Access denied' });
                }
            }
        ...
        const questions = await Question.find({ examId: attempt.examId }).populate('subjectId');
        ...
        const details = attempt.answers.map(ans => {
            ...
            let q = qMap[qIdStr] || questions.find(item => item._id.toString() === qIdStr);
            
            const correct = q ? (q.correctOptions && q.correctOptions.length > 0 ? q.correctOptions : (q.correctOption !== undefined && q.correctOption !== -1 ? [q.correctOption] : [])) : [];
            const selected = ans.selectedOptions && ans.selectedOptions.length > 0 ? ans.selectedOptions : (ans.selectedOption !== undefined && ans.selectedOption !== -1 && ans.selectedOption !== "-1" ? [Number(ans.selectedOption)] : []);
            ...
            return {
                question_text: q?.questionText || q?.question_text || q?.text || 'Question content missing',
                image_url: q?.imageUrl || '',
...
app.post('/api/notifications', requireLogin, async (req, res) => {
    try {
        ...
        if (req.session.role !== 'admin' && req.session.role !== 'teacher') {
            return res.status(403).json({ error: 'Only admins and teachers can send notifications' });
        }
...
app.post('/api/materials', requireLogin, async (req, res) => {
    try {
        ...
        const { class_id, subject_id, chapter_no, chapter_name, teacher_name, youtube_url, description, file_data, file_name } = req.body;

        if (!class_id || !subject_id || !chapter_no || !chapter_name) {
            return res.status(400).json({ error: 'class_id, subject_id, chapter_no, and chapter_name are required' });
        }
        ...
        if (req.session.role === 'teacher') {
            const teacherClassIds = await getTeacherClasses(req.session.userId);
            if (!teacherClassIds.map(id => id.toString()).includes(class_id)) {
                return res.status(403).json({ error: 'You can only upload materials for your assigned classes' });
            }
        }
...
app.get('/api/materials', requireLogin, async (req, res) => {
    try {
        ...
        if (!class_id || !subject_id) {
            return res.status(400).json({ error: 'class_id and subject_id query params are required' });
        }
        const materials = await StudyMaterial.find({ classId: class_id, subjectId: subject_id })
            .sort({ chapterNo: 1 });
        res.json({
...
app.get('/api/materials/subjects/:classId', requireLogin, async (req, res) => {
    try {
        ...
        res.json({ subjects: subjects.map(s => ({ id: s._id, name: s.name, code: s.code })) });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
app.get('/api/leaderboard/class/:classId', requireLogin, async (req, res) => {
    try {
        ...
        for (const s of students) {
            const marks = await Mark.find({ studentId: s._id }).populate('subjectId examId');
            const filteredMarks = await filterRealMarks(marks, true);
            
            // Overall Average
            const overallAverage = filteredMarks.length 
                ? Math.round(filteredMarks.reduce((sum, m) => sum + (m.marksObtained / (m.maxMarks || 100)) * 100, 0) / filteredMarks.length) 
                : 0;
            ...
            if (examList.length >= 2) {
                improvementTrend = examList[examList.length - 1].avg - examList[examList.length - 2].avg;
            }

            // Display Name resolution
            let displayName = s.name;
            if (s.useAlias) {
                displayName = s.alias ? s.alias : `Anonymous Student #${s.enrollmentNo.slice(-4)}`;
            }
    ...
    try {
        ...
        if (req.session.role === 'student') {
            ...
            if (!user || user.studentId?.toString() !== req.params.id) {
                return res.status(403).json({ error: 'Access denied: You can only update your own privacy settings.' });
            }
        ...
        res.json({ message: 'Privacy settings updated successfully', student: updatedStudent });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
mongoose.connect(MONGO_URI).then(async () => {
    ...
    server.on('error', (err) => {
        ...
        if (err.code === 'EADDRINUSE') {
            console.error(`[Server] Port ${PORT} is already in use. Kill the existing process first:\n  lsof -ti:${PORT} | xargs kill -9`);
        } else {
