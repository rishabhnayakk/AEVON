...
async function test() {
    ...
    const res = await fetch('http://localhost:6050/api/ai/predict/' + student._id.toString(), {
    ...
    const result = await analyzeStudent({
        name: student.name, className: "10A", enrollmentNo: "123",
        marks: [], subjectAverages: [], overallAverage: 50, attendanceRate: 50
    });
    console.log(result);
    process.exit(0);
}
